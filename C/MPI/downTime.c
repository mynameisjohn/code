...
int main(int argc, char *argv[]){
  ...
  int rank,size,localSize,i,j,time,tMax=0,tmp=0,master=0,count=0;
  float v=5.0,nu=0.2,mu=0.0,wMax=1.0,A=50.0/1000.0,B=1.0,C=0.0,D=20.0/1000.0;
...
//	for (j=0;j<NPOST;j++){
//	  dW[20*i+j] = nu*(pow((1.0-W[i*NPOST+j]),mu)*h*sPost[j] - pow(W[i*NPOST+j],mu)*xPost[j]*sPre[i]);
	  //	  printf("%lf\n",dW[20*i+j]);
//	}
	xPre[i] = solveRD(h,A,sPre[i]*(B-C*h));

	for (j=0;j<NPOST;j++)
	  xCum[j] += xPre[i]*W[NPOST*i+j];
	//printf("herp\n");
	if (time>0){
	MPI_Recv(sPost,NPOST,MPI_INT,master,17,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
 	MPI_Recv(xPost,NPOST,MPI_FLOAT,master,17,MPI_COMM_WORLD,MPI_STATUS_IGNORE);}
        for (j=0;j<NPOST;j++)
          dW[20*i+j] = nu*(pow((1.0-W[i*NPOST+j]),mu)*h*sPost[j] - pow(W[i*NPOST+j],mu)*xPost[j]*sPre[i]);
      }
    }
	...
	if (time>0){
	for (i=1;i<size;i++){
		MPI_Send(sPost,NPOST,MPI_INT,i,17,MPI_COMM_WORLD);
		MPI_Send(xPost,NPOST,MPI_FLOAT,i,17,MPI_COMM_WORLD);
	}}
      //MPI_Barrier(MPI_COMM_WORLD);
    }
