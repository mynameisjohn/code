...
bool SimpleShader::loadVert(std::string vertStr){
...
bool SimpleShader::loadFrag(std::string fragStr){
...
void SimpleShader::updateMV(){
   glUniformMatrix4fv(mMVMatAddr, 1, GL_FALSE, glm::value_ptr(mMVMat));
}

GLint SimpleShader::getPosAddr(){
	return mPosAddr;
}
